...
double MyAlgo::X(int u, int v, int req_no, int path_id, int middle){
    ...
    if((v == middle && u == (requests[req_no].get_node2() + node_num)) || (v == middle + node_num && u == (requests[req_no].get_node3() + node_num * 2))){
        return 0;
    }
...
void MyAlgo::initialize(int mid){
    ...
    for(unsigned  j = 0; j < requests.size(); j++){                      
        for(int k = 0; k < 3; k++){
            ...
            Y[mid][j][k][{node_num + node2, mid}] = 1;
            Y[mid][j][k][{ node_num * 2  + node3, mid + node_num}] = 1;
            Y[mid][j][k][{mid, node_num + node2}] = 1;
            Y[mid][j][k][{mid + node_num, node_num * 2  + node3}] = 1;
            for(int i = 0; i < node_num; i++){                           //找所有邊[i,it] // (v, u)
...
vector<int> MyAlgo::Dijkstra(int src, int dst, int req_no, int path_id, vector<pair<double,double>>&dist, vector<vector<int>> &n_graph){                           
    ...
    while(!pq.empty()){
        ...
        for(int neigh = 0; neigh < node_num * 3; neigh++){
            ...
            if(dist[cur_node].first + X(cur_node, neigh, req_no, path_id,dst % node_num) < dist[neigh].first) {
                dist[neigh].first = dist[cur_node].first + X(cur_node, neigh, req_no, path_id, dst% node_num);      //(1)store d
                dist[neigh].second = dist[cur_node].second + Y[dst % node_num][req_no][path_id][{cur_node, neigh}];
                parent[neigh] = cur_node;                                             //(1)store pred
                pq.push({dist[neigh].first, neigh});
            }
            if(dist[cur_node].first + X(cur_node, neigh, req_no, path_id, dst % node_num) == dist[neigh].first && dist[neigh].second > (dist[cur_node].second + Y[dst % node_num][req_no][path_id][{cur_node, neigh}])) {
                dist[neigh].first = dist[cur_node].first + X(cur_node, neigh,req_no,path_id,dst % node_num);      //(1)store d
                dist[neigh].second = dist[cur_node].second + Y[dst % node_num][req_no][path_id][{cur_node, neigh}];
                parent[neigh] = cur_node;                                             //(1)store pred
                pq.push({dist[neigh].first, neigh}); 
...
void MyAlgo::separation_oracle(int src, int dst, int req_no, int path_id, vector<vector<vector<int>>> &cur_tree, vector<vector<vector<double>>> &cur_label, vector<vector<int>> &n_graph){     
    ...
    for(int i = 0; i < node_num * 3; i++){                         //(4)compute lexmin
        ...
        for(int neigh = 0; neigh < node_num * 3; neigh++){
            ...
            double cur_obj_bar2 = dist[neigh].second + Y[dst % node_num][req_no][path_id][{neigh, i}] - dist[i].second;
            if(cur_obj_bar2 < 0){                                      //確認oba_bar2有沒有小於0
                double cur_theta = -(dist[neigh].first + X(neigh, i, req_no, path_id, dst % node_num) - dist[i].first) / cur_obj_bar2 ;
                if(cur_theta <= theta_table[i]){                       //計算對於i來說 theta最小的predecessor是誰
                    if(cur_theta < theta_table[i]){
                        theta_table[i] = cur_theta;
                        obj_bar1_table[i] = dist[neigh].first + X(neigh, i, req_no, path_id, dst % node_num) - dist[i].first;
    ...
    while(!pq.empty()){                                                 //(6)若priority_queue不為空
        ...
        for(int neigh = 0; neigh < node_num * 3; neigh++){
            ...
            double cur_obj_bar2 = dist[neigh].second + Y[dst % node_num][req_no][path_id][{neigh,min_i}] - dist[min_i].second;
            if(cur_obj_bar2 < 0){
                double cur_theta = -(dist[neigh].first + X(neigh, min_i, req_no, path_id, dst % node_num) - dist[min_i].first) / cur_obj_bar2 ;
                if(cur_theta <= theta_table[min_i]){
                    if(cur_theta < theta_table[min_i]){
                        theta_table[min_i] = cur_theta;
                        obj_bar1_table[min_i] = dist[neigh].first + X(neigh, min_i, req_no, path_id, dst % node_num) - dist[min_i].first;
        ...
        for(int neigh = 0; neigh < node_num * 3; neigh++){ //(14)~(19)對successor找最小的替換邊
            if(n_graph[min_i][neigh] == 0) continue;
            double temp_obj_bar2 = dist[min_i].second + Y[dst % node_num][req_no][path_id][{min_i,neigh}] - dist[neigh].second;
            if(temp_obj_bar2 < 0){
                double temp_obj_bar1 = dist[min_i].first + X(min_i,neigh,req_no,path_id, dst % node_num) - dist[neigh].first;
    ...
    if(d1 * exp(d2) <  smallest_U){
        ...
        cur_tree[path_id].push_back(best_path);
    }
...
void MyAlgo::create_graph(vector<vector<vector<vector<int>>>> &n_graph){
    ...
    for(int i = 0; i < node_num; i++){
        for(int j = 0; j < requests.size(); j++){
            ...
            // n_graph[i][j][node_num + node2][i] = 1;
            // n_graph[i][j][node_num * 2 + node3][i + node_num] = 1;
            for(int k = 0; k < 3; k++){
...
vector<vector<int>> MyAlgo::parse_tree(vector<int> &tree, int middle, int node_num){
    vector<vector<int>> parseTree(3, vector<int>());
    
    int j = 0;
    for(int i = 0; i < tree.size(); i++){
        if((tree[i] % node_num) != middle){
            parseTree[j].push_back(tree[i] % node_num);
        }else{
            parseTree[j].push_back(tree[i] % node_num);
            j++;
        }
    ...
    while(obj < 1){
        ...
        for(int middle = 0; middle < node_num; middle++){
            for(unsigned int i = 0; i < requests.size(); i++){
                ...
                int node1 = requests[i].get_node1();
                int node2 = requests[i].get_node2();
                int node3 = requests[i].get_node3();
               
                if(middle == node1 || middle == node2 || middle == node3){continue;}      //middle_node不可以是三個端點

                separation_oracle(node1, middle + node_num * 2, i, 0, cur_tree, cur_label, n_graph[middle][i]);  
                // separation_oracle_ori(node1, middle + node_num * 2, i, 0, n_graph[middle][i]);
                int best_index = -1;
                
                for(int j = 0; j < cur_tree[0].size(); j++){

                    // for(int k = 0; k < cur_tree[0][j].size(); k++){
                    //     cout <<cur_tree[0][j][k] % node_num << " " ;
                    // }

                    if(smallest_U > cur_label[0][j][2]){
                        smallest_U = cur_label[0][j][2];
                        best_index = j;
                        
                    }
                    // cout << " "  << cur_label[0][j][2] << endl;
                ...
                if(smallest_U < global_U){
                    global_U = smallest_U;
                    req_no = i;
                    best_tree = parse_tree(cur_tree[0][best_index], middle, node_num);
...
vector<int> MyAlgo::Dijkstra_ori(int src, int dst, int req_no, int path_id , vector<vector<int>> &n_graph){ 
    const double INF = numeric_limits<double>::infinity();
    //const double INF = 100;
    int n = graph.get_size();
    vector<double> distance(n * 3 + 1, INF);
    vector<int> parent(n * 3 + 1, -1);
    vector<bool> used(n * 3 + 1, false);
    priority_queue<pair<double, int>, vector<pair<double, int>>, greater<pair<double, int>>> pq;
    ...
    while(!pq.empty()) {
        ...
        if(used[cur_node]) continue;
        used[cur_node] = true;
        for(int neighbor = 0; neighbor < n * 3; neighbor++){
            if(n_graph[cur_node][neighbor] == 0) continue;
...
void MyAlgo::separation_oracle_ori(int src,int dst,int req_no, int path_id, vector<vector<int>> &n_graph){     
    vector<int> SPT;                  //nodes' parent in the spanning tree
    vector<int> best_path;
    double best_len; 
    SPT = Dijkstra_ori(src, dst, req_no, path_id, n_graph);                               //the first SPT is get by dijkstra
    int cur_node = src;                                     //counting the first path's U(X,Y)=c* e^r
    double c = 0;                                           //c=SUM[u,v]:alpha(u)+alpha(v)+beta(u,v)==X[u,v]
    double r = 0;           
    int n = graph.get_size();                                //r=SUM[u,v]:-ln(Pr(u,v))==Y[u,v]

    while(cur_node != dst){
        c += X(SPT[cur_node], cur_node,req_no,path_id,dst);  
        r += Y[dst % n][req_no][path_id][{SPT[cur_node],cur_node}];  
    ...
    while(1){
        ...
        for(int i = 0; i < n * 3; i++){                 //creating many new SPT
            for(int neighbor = 0; neighbor < n * 3; neighbor++){
                if(n_graph[i][neighbor] == 0) continue;
                double temp1 = 0, temp2 = 0;
                if(SPT[i] == neighbor || SPT[neighbor] == i){      // checking used edge or unused
                    continue;   
                }else{                                             // if unused
                    temp1 = X(i, neighbor,req_no,path_id,dst);
                    temp2 = Y[dst % n][req_no][path_id][{i, neighbor}];
                    int cur_node = i;
                    while(cur_node != dst){
                        temp1 += X(cur_node, SPT[cur_node],req_no,path_id,dst);
                        temp2 += Y[dst % n][req_no][path_id][{cur_node, SPT[cur_node]}];
                        cur_node = SPT[cur_node];
                    } 
        ...
        for(unsigned int i = 0; i < new_path.size() - 1; i++){
            if(new_path[i] < new_path[i+1]){                        //[can alter]no need if else
                ...
                r += Y[dst % n][req_no][path_id][{new_path[i], new_path[i+1]}];
            }
            else{
                c += X(new_path[i+1], new_path[i],req_no,path_id,dst);  
                r += Y[dst % n][req_no][path_id][{new_path[i+1], new_path[i]}];           
            }
